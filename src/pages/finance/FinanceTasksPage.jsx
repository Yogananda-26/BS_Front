...
const SubmitTaskModal = ({ task, show, onHide, onSubmitted }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const id = task?.pmTaskId || task?.assignedTaskId || task?.id;
      await financeService.submitTask(id, description.trim());
          ...
          <div className="bg-light rounded-3 p-3 mb-3">
            <div className="d-flex align-items-start gap-2">
              <div className="flex-grow-1">
                ...
                <div className="fw-semibold font-monospace small text-primary">{task.pmTaskId || task.id}</div>
                <div className="small text-dark mt-1">{task.description}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-4">
            ...
            <Form.Control
              ...
              placeholder="Describe what was completed, findings, or any notes for the PM..."
              className="rounded-3"
              required
            />
...
const TaskCard = ({ task, onComplete }) => {
  ...
  return (
    <div
      className={`bg-white rounded-4 shadow-sm border-start border-4 border-${sta.bg} overflow-hidden`}
    ...
    >
      ...
      <div className="p-4">
        <Row className="align-items-start g-3">
          ...
          <Col xs="auto">
            <div
              className={`bg-${sta.bg} bg-opacity-10 rounded-3 d-flex align-items-center justify-content-center`}
          ...
          <Col>
            ...
            <div className="d-flex align-items-center gap-2 flex-wrap mb-2">
              <span className="fw-bold font-monospace text-primary small">{task.pmTaskId || task.id || "—"}</span>
            ...
            <p className="mb-2 text-dark" style={{ lineHeight: 1.5 }}>
              {task.description || task.taskDescription || "No description provided."}
            </p>
            ...
            <div className="d-flex flex-wrap gap-3 small text-muted">
              {(task.assignedBy || task.assignedByName) && (
                <span className="d-flex align-items-center gap-1">
                  <FaUser size={11} /> {task.assignedByName || task.assignedBy}
                </span>
              )}
              {(task.plannedEnd || task.actualEnd || task.taskDeadline || task.plannedEndDate || task.endDate || task.deadline || task.dueDate || extractPlannedEnd(task.description || task.taskDescription) || task.syncedAt || task.assignedAt) && (
                <span className="d-flex align-items-center gap-1">
                  <FaCalendarAlt size={11} />
                  {(task.plannedEnd || task.actualEnd || task.taskDeadline || task.plannedEndDate || task.endDate || task.deadline || task.dueDate || extractPlannedEnd(task.description || task.taskDescription)) ? "Due: " : "Synced: "}
                  {formatDate(task.plannedEnd || task.actualEnd || task.taskDeadline || task.plannedEndDate || task.endDate || task.deadline || task.dueDate || extractPlannedEnd(task.description || task.taskDescription) || task.syncedAt || task.assignedAt)}
                </span>
              )}
              {task.projectName && (
                <span className="d-flex align-items-center gap-1">
      ...
      {expanded && (
        <div className="px-4 pb-4 border-top pt-3 bg-light bg-opacity-50">
          <Row className="g-3">
            <Col md={3} xs={6}>
              <div className="small text-muted fw-bold mb-1">PM TASK ID</div>
              <div className="small fw-semibold font-monospace">{task.pmTaskId || "—"}</div>
            </Col>
            <Col md={3} xs={6}>
            ...
            <Col md={3} xs={6}>
              ...
              <div className="small fw-semibold">{task.assignedBy || "—"}</div>
            </Col>
            <Col md={3} xs={6}>
              <div className="small text-muted fw-bold mb-1">DUE DATE</div>
              <div className="small fw-semibold">{formatDate(task.plannedEnd || task.actualEnd || task.taskDeadline || task.plannedEndDate || task.endDate || task.deadline || task.dueDate || extractPlannedEnd(task.description || task.taskDescription) || task.syncedAt)}</div>
            </Col>
            {task.completedAt && (
              <Col md={3} xs={6}>
                <div className="small text-muted fw-bold mb-1">COMPLETED AT</div>
                <div className="small fw-semibold">{formatDate(task.completedAt)}</div>
              </Col>
            )}
...
const FinanceTasksPage = () => {
  ...
  const silentRefresh = async () => {
    try {
      await financeService.syncTasks();
      const data = await financeService.getTasks();
      setTasks(Array.isArray(data) ? data : data?.content || []);
    } catch { /* silent */ }
  };
    ...
    <div className="p-4">
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            Finance tasks assigned by Project Manager.
            {pendingCount > 0 && (
              <Badge bg="danger" pill className="ms-2">{pendingCount} pending</Badge>
            )}
          </p>
        </div>
        <AppButton
        ...
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <Badge
                bg={tab === "PENDING" && count > 0 ? "danger" : (activeTab === tab ? "light" : "secondary")}
                text={activeTab === tab && tab !== "PENDING" ? "dark" : undefined}
                pill
                className="ms-2"
              >
                {count}
      ...
      {loading ? (
        ...
        <div className="d-flex flex-column gap-3">
          {filtered.map((task) => (
            <TaskCard
              key={task.assignedTaskId || task.id || task.taskId || task.pmTaskId}
              task={task}
              onComplete={setSubmitTask}
            />
          ))}
        </div>
      )}
